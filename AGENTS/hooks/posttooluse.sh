...
# @see AGENTS.md, AGENTS/skills/check-comments/, AGENTS/skills/check-wayfinders/, .claude/settings.json
...
SKILLS=$(cd "$(dirname "${BASH_SOURCE[0]}")/../skills" 2>/dev/null && pwd || true)
# a missing directory means a rename moved the validators, which would silently disable this hook
if [ ! -d "$SKILLS" ]; then
  echo "posttooluse: no AGENTS/skills beside this hook; comment and wayfinder linting is off" >&2
  exit 0
fi
...
FINDINGS=$(
  {
    bash "$SKILLS/check-comments/check-comments.sh" "$FILE" 2>/dev/null || true
    bash "$SKILLS/check-wayfinders/check-wayfinders.sh" "$FILE" 2>/dev/null || true
