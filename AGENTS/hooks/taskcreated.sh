...
# @see AGENTS.md, AGENTS/skills/doc-logs/SKILL.md, AGENTS/hooks/taskcompleted.sh, docs/logs/
...
jq -n --arg ctx "check the most recent thread in $TODAYS_LOG; if this new task is unrelated to it, start a new thread before continuing (see AGENTS/skills/doc-logs/SKILL.md)" \
