...
# ARTIFACT
# - `audits/<kind>/YYYY-MM-DD.md`, one file per day and per kind
...
# RUN
# - defaults to every file in `audits/`; pass files or a directory to scope it
# - `--strict` promotes warnings to errors, `--keep` preserves scratch; exits 1 on any error
# - ERROR breaks a rule the doc states outright; WARN names a smell the doc tolerates
# @see AGENTS.md, plugins/operator/skills/settings/SKILL.md, plugins/gitgud/skills/audit/SKILL.md, plugins/operator/skills/settings/SKILL.md, audits/, plugins/operator/shared/secrets.sh
...
STRICT=0
KEEP=0
TEMPLATE="plugins/operator/skills/settings/SKILL.md"
ARTIFACTS="audits"
...
if [ ${#AUDITS[@]} -eq 0 ]; then
  ...
  # credentials keeps its own shape and its own validator, so it is not an audit archive
  AUDITS=()
  for found in "$ARTIFACTS"/*/*.md; do
    case "$found" in "$ARTIFACTS"/credentials/*) continue;; esac
    [ -f "$found" ] && AUDITS+=("$found")
  done
fi
...
for path in "${AUDITS[@]}"; do
  if [ -d "$path" ]; then
    # an audit now sits one level down, under the directory naming its kind
    for nested in "$path"/*.md "$path"/*/*.md; do [ -f "$nested" ] && EXPANDED+=("$nested"); done
  elif [ -f "$path" ]; then EXPANDED+=("$path")
  else echo "fatal: no such audit file: $path" >&2; exit 1; fi
done
if [ ${#EXPANDED[@]} -eq 0 ]; then echo "fatal: no audit files found under ${AUDITS[*]}" >&2; exit 1; fi
AUDITS=("${EXPANDED[@]}")
...
check_filename() {
  ...
  if ! printf '%s' "$base" | grep -qE '^[0-9]{4}-[0-9]{2}-[0-9]{2}\.md$'; then
    err "$file" 1 filename "one audit file per day, named YYYY-MM-DD.md under its kind"
  fi
  case "$dir" in
    "$ARTIFACTS"/*|"./$ARTIFACTS"/*|*"/$ARTIFACTS"/*) ;;
    *) warn "$file" 1 location "an audit lives in $ARTIFACTS/<kind>/, never loose in $ARTIFACTS/";;
  esac
}
...
check_header() {
  ...
  expected="# $ARTIFACTS/$(basename "$(dirname "$file")")/$(basename "$file")"
...
check_entries() {
  ...
  # the date is the filename and the kind is the directory above it, so read one from each
  base=$(basename "$file" .md)
  date=$(printf '%s' "$base" | cut -c1-10)
  kind=$(basename "$(dirname "$file")")
...
check_width() {
  local file=$1 lineno=0 line fenced=0
  while IFS= read -r line; do
    lineno=$((lineno + 1))
    # telemetry is pasted unedited by contract, so reflowing it to fit would break the other rule
