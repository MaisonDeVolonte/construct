...
# ARTIFACT
# - `audits/credentials/YYYY-MM-DD.md`, one file per day, written by `/test-credentials`
...
# RUN
# - defaults to every file in `audits/credentials/`; pass files or a directory to scope it
# - `--strict` promotes warnings to errors; exits 1 on any error
# @see AGENTS.md, plugins/operator/skills/credentials/SKILL.md, plugins/operator/skills/credentials/SKILL.md, plugins/operator/skills/credentials/credentials.sh, audits/credentials/, plugins/operator/shared/secrets.sh
...
if [ ${#TARGETS[@]} -eq 0 ]; then TARGETS=("audits/credentials"); fi
