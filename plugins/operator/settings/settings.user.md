 ...
 * @see plugins/operator/settings/settings.user.json, plugins/operator/settings/settings.project.md, plugins/operator/settings/settings.managed.md, plugins/operator/settings/settings.cli.md, plugins/operator/skills/settings/settings.sh, AGENTS.md
 */
```
...
- `/gitgud:deliver` hands its block to the user, so the credential never reaches an agent-run command
...
- `plugins/gitgud/shared/handover.sh` is written against this list, so a sidecar cannot drift out of it
...
"Bash(git clean*)", "Bash(git add*)",
"Bash(git rm -f*)", "Bash(git rm --force*)",
...
- `/gitgud:nuke` hands these back rather than running them, which is the pattern to copy
- `git rm` and `git mv` moved from deny to ask on 2026-08-07, since a refactor is dozens of them
  and both are recoverable from HEAD; only the force forms stay denied, because `git rm -f`
  discards uncommitted content and is the one shape git cannot give back
