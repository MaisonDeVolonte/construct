...
# - the note follows the `/operator:logs` template rather than inventing a shape
...
# @see plugins/operator/skills/logs/SKILL.md, plugins/operator/hooks/hooks.json, .construct/operator/logs/
...
jq -n --arg reason "add a note to $TODAYS_LOG (see /operator:logs); notify user" \
