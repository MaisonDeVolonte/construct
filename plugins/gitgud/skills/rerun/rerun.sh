#!/bin/bash
# ============================================
# @file rerun.sh - stale-pr ci refresh sidecar
# ============================================
# @description
# PAIR
# - sidecar for `/gitgud:rerun` — re-runs ci on a stale pr against the current default branch
# - reach for it when `/gitgud:deliver` left a pr computed against a trunk that has since moved
...
# @see plugins/gitgud/skills/rerun/SKILL.md, plugins/gitgud/skills/deliver/SKILL.md, plugins/gitgud/shared/handover.sh, .claude/skills/validate-skills/SKILL.md
...
if [ -z "$CURRENT_BRANCH" ]; then
  echo "fatal: detached HEAD (run /gitgud:rerun from the pr branch)" >&2; exit 1; fi
...
if [ -z "$PR_INFO" ]; then
  echo "fatal: no open pr for the current branch (run /gitgud:rerun from the pr branch)" >&2; exit 1; fi
