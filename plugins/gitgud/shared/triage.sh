...
# @description
# PAIR
# - library and standalone read, paired with no skill: it probes branch state and prints telemetry
# - read-only in every sense; it writes no file and owns no artifact kind
# - `/gitgud:audit`, `/gitgud:prune` and `/gitgud:nuke` all reach it; one read serves all three
# - team probes ride curl + bearer against the github rest api, the one path the sandbox serves
# READS
# - the caller labels ghost branches, local clutter, rebase-absorbed branches, and conflict risk
# - `merged` is a patch-id read and `absorbed` is a tree read, so only `absorbed` clears a rebase
# - the git archive belongs to `/gitgud:audit`, which names its own target and grades what lands
# @see plugins/gitgud/skills/audit/audit.sh, plugins/gitgud/shared/handover.sh, plugins/gitgud/skills/prune/prune.sh, plugins/gitgud/skills/nuke/nuke.sh
