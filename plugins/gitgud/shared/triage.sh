...
# - `/gitgud:prune` and `/gitgud:nuke` both call it, since branch triage is the same read for all three
# TRIGGER
...
# - it then appends that report to `audits/git/YYYY-MM-DD.md`, in the `/operator:settings` shape
# - the sidecar names today's audit path and count, and never creates the file itself
# @see AGENTS.md, plugins/gitgud/skills/audit/SKILL.md, /operator:settings, plugins/gitgud/shared/handover.sh, tools/check-skills/README.md, audits/
...
TODAYS_AUDIT="audits/git/$(date +%Y-%m-%d).md"
