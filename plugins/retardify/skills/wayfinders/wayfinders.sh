...
# @see AGENTS.md, plugins/retardify/skills/wayfinders/SKILL.md, plugins/retardify/skills/comments/SKILL.md, plugins/retardify/skills/comments/comments.sh, plugins/retardify/skills/todo/todo.sh, plugins/retardify/shared/secrets.sh
...
STRICT=0
KEEP=0
TEMPLATE="plugins/retardify/skills/wayfinders/SKILL.md"
...
is_generated() {
  case "$1" in
    *tests/report*|*tests/results*|*.next*|*node_modules*) return 0;;
    docs/logs*|docs/plans*|docs/guides*|docs/honest*|docs/insights*|docs/graphs*|audits/*) return 0;;
    *) return 1;;
  esac
}
...
TODAYS_AUDIT="audits/wayfinders/$(date +%Y-%m-%d).md"
if [ -f "$AUDIT_ROOT/$TODAYS_AUDIT" ];
then AUDIT_COUNT=$(grep -c '^## Wayfinders Audit #' "$AUDIT_ROOT/$TODAYS_AUDIT" || true)
