...
# - written by `/retardify:review` only, so one file holds many scorecards
...
# @see AGENTS.md, plugins/retardify/skills/review/SKILL.md, plugins/retardify/skills/review/SKILL.md, plugins/retardify/skills/review/review.sh, docs/honest/, plugins/retardify/shared/secrets.sh
...
check_filename() {
  ...
  case "$dir" in
    ...
    *) warn "$file" 1 location "scorecard archives live in $ARTIFACTS/";;
  esac
}
