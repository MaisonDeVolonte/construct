#!/bin/bash
# ================================================================================
# @file todo.sh - opportunity-scan sidecar: runs the scan, then grades the reports
# ================================================================================
# @description
# PAIR
# - the only sidecar for `/retardify:todo`, which owns both halves of its own artifact
...
check_filename() {
  ...
  case "$dir" in
    ...
    *) warn "$file" 1 location "report artifacts live in $ARTIFACTS/";;
  esac
}
...
# getting skipped, which is the only thing the artifact is for
check_subsections() {
